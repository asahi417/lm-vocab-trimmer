...
for la in ['ja', 'ru', 'fr', 'es', 'it', 'ko']:
    ...
    data.update(download(f"{la}.raw.json", url=f"https://huggingface.co/lmqg/mt5-small-{la}quad-qg/raw/main/eval/metric.first.sentence.paragraph_answer.question.lmqg_qg_{la}quad.default.json"))
    full_data.append(data)

    data = {"language": la, 'size': max_vocab[la], "type": "Post-FT (FULL)"}
    data.update(download(f"{la}.json", url=f"https://huggingface.co/vocabtrimmer/mt5-small-{la}quad-qg-trimmed-{la}/raw/main/eval/metric.first.sentence.paragraph_answer.question.lmqg_qg_{la}quad.default.json"))
    full_data.append(data)

    data = {"language": la, 'size': max_vocab[la], "type": "Pre-FT (FULL)"}
    data.update(download(f"{la}.ft_trimmed.json", url=f"https://huggingface.co/vocabtrimmer/mt5-small-trimmed-{la}-{la}quad-qg/raw/main/eval/metric.first.sentence.paragraph_answer.question.lmqg_qg_{la}quad.default.json"))
    full_data.append(data)
    ...
    for v_size in [5000, 10000, 15000, 30000, 60000, 90000, 120000]:
        ...
        data.update(download(f"{la}.{v_size}.ft_trimmed.json", url=f"https://huggingface.co/vocabtrimmer/mt5-small-trimmed-{la}-{v_size}-{la}quad-qg/raw/main/eval/metric.first.sentence.paragraph_answer.question.lmqg_qg_{la}quad.default.json"))
        full_data.append(data)

        data = {"language": la, 'size': v_size, "type": "Post-FT"}
        data.update(download(f"{la}.{v_size}.json", url=f"https://huggingface.co/vocabtrimmer/mt5-small-{la}quad-qg-trimmed-{la}-{v_size}/raw/main/eval/metric.first.sentence.paragraph_answer.question.lmqg_qg_{la}quad.default.json"))
        full_data.append(data)

df = pd.DataFrame(full_data)[['Bleu_4', 'METEOR', 'ROUGE_L', 'BERTScore', 'MoverScore', "language", "size", "type"]]
...
for m in ['METEOR', 'ROUGE_L', 'BERTScore', 'MoverScore', 'Bleu_4']:
    ...
    main_df = main_df[['Vocab (Param)', "Trimming"] + [c for c in main_df.columns if c not in ['Vocab (Param)', "Trimming"]]]
    ...
    main_df = (main_df.pivot(columns="type", values=m) * 100).round(1)
