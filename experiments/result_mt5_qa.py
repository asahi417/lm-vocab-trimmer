...
for m in ["AnswerF1Score", "AnswerExactMatch"]:
    ...
    main_df['Vocab (Param)'] = main_df.pop("size")
    main_df['Model'] = main_df.pop("type")
    main_df = main_df[['Model', 'Vocab (Param)'] + [c for c in main_df.columns if c not in ['Model', 'Vocab (Param)']]]
    print(f"** metric: {m} **")
